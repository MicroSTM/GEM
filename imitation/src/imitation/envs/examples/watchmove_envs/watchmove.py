...
from imitation.envs.examples.watchmove_envs.watchmove_utils import *
...
class WM(gym.Env):
    def __init__(
        ...
        action_type="force",
        maze_sampler=None,
        goals=[["LMA", 0, (15, 15), 1], [None], [None]],
        strengths=[0, 0, 0],
        sizes=[0, 0, 0],  # [1, 1, 1],#
        densities=[1, 1, 1],  # [0, 0, 0],
        init_positions=[18, 10, 12],  # [7, 10, 12],
        action_space_types=[0, 0, 0],
        costs=[0, 0, 0],
        temporal_decay=[0, 0, 0],
        visibility=[1, 1, 1],
        num_agents=3,
        num_items=0,
        ...
        n_particles=50,
        belief_prior=0.5,
        init_agent_angles=[0.0, 0.0, 0.0],
        use_all_init_grid_pos=False,
        human_simulation=False,
        no_done=False,
        act_space_per_agent=False,
        colors=[0, 1, 2],
        shapes=[0, 0, 0],
        G=None,
        predicates=[],
        ...
        # random.seed(123)
        self.maze_sampler = MazeSampler()  # maze_sampler
        self.maze_sampler.gen_env_defs()
        ...
        self.expert_subgoals = expert_subgoals

        low_vals = (
            [6.0, 2.0, -360.0]
            + [0 for _ in range(len(ENT_COLOR_LIST))]
            + [0 for _ in range(len(SHAPE))]
        )
        high_vals = (
            [26.0, 22.0, 360.0]
            + [1 for _ in range(len(ENT_COLOR_LIST))]
            + [1 for _ in range(len(SHAPE))]
        )
        self.observation_space = gym.spaces.Box(
            low=np.array(low_vals * self.num_entities),
            high=np.array(high_vals * self.num_entities),
            dtype=np.float32,
        )  # [pos_x,pos_y,angle,color-one-hot,shape-one-hot] for each entity (color is based on ENT_COLOR_LIST)
        self.state_dim = len(low_vals)
        self._action_space = [
            "up",
            "down",
            "left",
        ...
        self.action_size = len(self._action_space)
        self.action_space = gym.spaces.Discrete(self.action_size)
        if act_space_per_agent:
            self.action_space = gym.spaces.Discrete(self.action_size * self.num_agents)
        if self.enable_renderer:
            self.screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT), 0, 32)
            pygame.display.set_caption("Maze_v2")
    ...
    def get_action_id(self, action):
        for action_id, a in enumerate(self._action_space):
            if action == a:
                return action_id
        return None
    ...
    def setup(
        ...
        for agent_id in range(self.num_agents):
            ...
            ag_shape = SHAPE[self.shapes[agent_id]]
            if ag_shape == "circle":
            ...
            # self.trajectories[agent_id] = tuple(self.get_full_state()) #[(x, y, 0, 0, 0)]
    ...
    def start(self, replay=False, record_dir=None):
        ...
        for agent_id in range(self.num_agents):
            self.trajectories[agent_id] = [
                tuple(self.get_ag_state(agent_id))
            ]  # [(x, y, 0, 0, 0)]
    ...
    def reset(self):
        # choose random pos for object 0
        # self.init_positions[1] = random.choice([16, 2, 15])
        # self.init_positions[1] = 2
        self.setup()
        self.start()
        return self.get_full_state()

    # returns list of states for observed entities by agent agent_id.
    # the order is:
    # state tuples/None (unobserved entities are None) - agent_id, other agent, item0, item1
    # pos tuples - landmark centers
    # binary - agent_id
    def _get_obs(self, agent_id):
        observed_entity_states = [None for entity in range(self.num_entities)]
        # observed_entity_states = [None for entity in range(self.num_entities+len(self.landmark_centers))]

        # by observation - for partialy observable setting. TODO why are there Nones for full observation?
        #  agent = self.agents[agent_id]
        # for obs in agent.last_observations.keys():
        #     entity_type, entity_id = obs
        #     if entity_type == 'agent':
        #         state = self.agent_states[entity_id]
        #         egocentric_id = 0 if entity_id == agent_id else 1
        #         observed_entity_states[egocentric_id] = (state['pos'][0], state['pos'][1], state['vel'][0], state['vel'][1], state['angle'])
        #     elif obs == 'item':
        ...
        # by entity. outputs all objects, no partial view.
        for ag_id in range(self.num_agents):
            state = self.agent_states[ag_id]
            egocentric_id = 0 if ag_id == agent_id else 1
            observed_entity_states[egocentric_id] = [
                state["pos"][0],
                state["pos"][1],
                state["vel"][0],
                state["vel"][1],
        ...
        for it_id in range(self.num_items):
            state = self.item_states[it_id]
            observed_entity_states[self.num_agents + it_id] = [
                state["pos"][0],
                state["pos"][1],
                state["vel"][0],
                state["vel"][1],
        ...
        print("observed_entity_states", observed_entity_states)
        observed_entity_states = list(np.array(observed_entity_states).flatten())
        assert None not in observed_entity_states
        return observed_entity_states  # + [coor for center in self.landmark_centers for coor in center] #+ [agent_id]

    def get_full_state(self):
        obs_n = [None for entity in range(self.num_entities)]
        ncolors = len(ENT_COLOR_LIST)
        nshapes = len(SHAPE)
        for ag_id in range(self.num_agents):
            state = self.agent_states[ag_id]
            one_hot_color = [0 for _ in range(ncolors)]
            one_hot_color[self.colors[ag_id]] = 1
            one_hot_shape = [0 for _ in range(nshapes)]
            one_hot_shape[self.shapes[ag_id]] = 1
            obs_n[ag_id] = (
                [
                    state["pos"][0],
                    state["pos"][1],
        ...
        for it_id in range(self.num_items):
            state = self.item_states[it_id]
            one_hot_color = [0 for _ in range(ncolors)]
            one_hot_color[self.colors[self.num_agents + it_id]] = 1
            one_hot_shape = [0 for _ in range(nshapes)]
            one_hot_shape[self.shapes[self.num_agents + it_id]] = 1
            obs_n[self.num_agents + it_id] = (
                [
                    state["pos"][0],
                    state["pos"][1],
                    state["angle"],
        ...
        obs_n = list(np.array(obs_n).flatten())
        assert None not in obs_n
        # print('obs_n',obs_n)

        return np.array(obs_n)

    def get_full_state_tmp(self):
        obs_n = [None for entity in range(self.num_entities)]
        ncolors = len(ENT_COLOR_LIST)
        nshapes = len(SHAPE)
        for ag_id in range(self.num_agents):
            state = self.tmp_agent_states[ag_id]
            one_hot_color = [0 for _ in range(ncolors)]
            one_hot_color[self.colors[ag_id]] = 1
            one_hot_shape = [0 for _ in range(nshapes)]
            one_hot_shape[self.shapes[ag_id]] = 1
        ...
        for it_id in range(self.num_items):
            state = self.tmp_item_states[it_id]
            one_hot_color = [0 for _ in range(ncolors)]
            one_hot_color[self.colors[self.num_agents + it_id]] = 1
            one_hot_shape = [0 for _ in range(nshapes)]
            one_hot_shape[self.shapes[self.num_agents + it_id]] = 1
    ...
    def reset_history(self):
        ...
        self.actions = [None] * self.num_agents
        for agent_id in range(self.num_agents):
            self.trajectories[agent_id] = [tuple(self.get_ag_state(agent_id))]
    ...
    def check_attachable(self, agent_id, item_id=None, threshold=0.5):
        ...
        if (
            self._action_space_types[agent_id] == 2
    ...
    def get_action_space(self, agent_id):
        ...
        return self._action_space  # no attach, detach, stop, noforce

        num_actions = len(self._action_space) - self.action_space_types[agent_id]
        if not self.attached[agent_id]:
            num_actions = min(num_actions, len(self._action_space) - 1)
        # else:
        ...
        if self.goals[agent_id][0] in ["RO", "GE", "TE"]:
            if self.goals[agent_id][0] == "RO":
                ...
                if not self.attached[agent_id]:
                    ...
                    num_actions = min(num_actions, len(self._action_space) - 2)
        ...
        #     return ['turnleft', 'turnright', 'up', 'down', 'stop', 'noforce']
            ...
            num_actions = min(num_actions, len(self._action_space) - 2)
        # print('get_action_space', self.goals[agent_id], self._action_space[:num_actions])
        return self._action_space[:num_actions]
    ...
    def send_action(self, agent_id, action):
        ...
        # print('self.action_space_types',self.action_space_types,'agent_id',agent_id)
        if action not in (
            self._action_space[: -self.action_space_types[agent_id]]
        ...
        # print('send_action f',f)
        # print('send_action p',p)
        self.agents[agent_id].ApplyLinearImpulse(f * 0.05, p, True)  # TODO human player
    ...
    def step(self, actions, replay=False, record_dir=None, planning_agent_id=None):
        """apply one step and update the environment"""
        # planning_agent_id for planner. for PPO discrete action space, indicates who the planning agent is and what action to take.

        if planning_agent_id is None:  # PPO, action space idx
            planning_agent_id = actions // self.action_size
            convert_actions = [None for _ in range(self.num_agents)]
            for agent_id in range(self.num_agents):
                if (
                    planning_agent_id == agent_id
                ):  # convert from number idx to action name
                    convert_actions[agent_id] = self._action_space[
                        actions // self.num_agents
                    ]
            actions = convert_actions

        for agent_id in range(self.num_agents):
            self.send_action(agent_id, actions[agent_id])
        ...
        for agent_id, state in enumerate(self.agent_states):
            ...
            self.trajectories[agent_id].append(tuple(self.get_ag_state(agent_id)))
        ...
        for t in range(self.NUM_STEPS_PER_TICK - 1):
            ...
            for agent_id in range(self.num_agents):  # real action
        ...
        for agent_id in range(self.num_agents):
            ...
            self.agents[agent_id].linearVelocity.x = 0
            self.agents[agent_id].linearVelocity.y = 0
            # self.repeat_actions[agent_id] = None
        ...
        obs_n = self.get_full_state()  # self._get_obs(0)  # 0-agent id
        reward_n = self.get_reward_state(
            agent_id=planning_agent_id,  # actions//self.action_size,
            curr_state=self.get_state(),
            action=actions[planning_agent_id],
        )
        done_n = self._get_done()
        info_n = {"ob": obs_n, "rew": reward_n, "done": done_n}
    ...
    def convert_obs2state(self, obs):
        return {
            "pos": tuple(obs[:2]),
            "vel": (0, 0),  # lost information (not part of obs)
            "angle": obs[2],
            "angleVel": 0,
            "attached": None,
        }

    def convert_fullobs2state(self, fullobs):
        fullstate = []
        for ent in range(self.num_entities):
            fullstate.append(
                self.convert_obs2state(
                    fullobs[ent * self.state_dim : (ent + 1) * self.state_dim]
                )
            )
        return fullstate

    # def convert_state2obs(self, s):
    #     pass #TODO
    ...
    # TODO change to 0.05
    def terminal(self, agent_id=None):
        """check if subgoal is achieved"""
        if agent_id is not None:
            return (
                self.get_reward_state(
                    agent_id, self.get_state(), "stop", None, None, self.goals[agent_id]
                )
                > -0.02
            )
        else:
            # print('terminal',self.goals)
            for agent_id, goal in enumerate(self.goals):
                ...
                if (
                    ...
                    # print('get_reward_state',goal,agent_id, self.get_state(),
                    #                     self.get_reward_state(agent_id, self.get_state(), 'stop', None, None, goal))
                    return False
            return True
        # return False

    def _terminal_tmp(self, agent_id=None):
        """check if subgoal is achieved"""
        if agent_id is not None:
            return (
                self.get_reward_state(
            ...
            for agent_id, goal in enumerate(self.goals):
                ...
                if (
                    self.get_reward_state(
                        agent_id, self._get_state_tmp(), "stop", None, None, goal
                    )
                    <= -0.02
                ):
                    return False
    ...
    def get_reward_state(
        ...
        if goal is not None and len(goal) == 2:
            ...
            # TODO how to deal otherwise?
            if (
                goal[0] is None
            ):  # no input, env goals are None --> has no goal, penalty for moving
                return -np.float32(COST[-1])
            if goal[0] == "stop":
            ...
            if goal[0] == "ROT":
                agent_deg = (
                    math.degrees(self.agents[agent_id].angle) % 360
                )  # rad to deg
                # normalize by 50 to fit -0.02 thres (means degrees are within dist 10)
                # print("ROT rew ",(-(np.abs(goal[2]-agent_deg)%360))/50)
                # pdb.set_trace()
                return (-(np.abs(goal[2] - agent_deg) % 360)) / 50 - COST[
                    self.costs[agent_id]
                ]

            """TODO: RA & RO"""
            if goal[0] == "LMA":
                if len(goal[2]) == 2:  # already pos
                    dist_to_goal = self._get_dist_pos(agents_pos[goal[1]], goal[2])
                    # print('LMA pos dist_to_goal',dist_to_goal)
                else:  # from POS list
                    dist_to_goal = self._get_dist_pos(
    ...
    def _get_done(self):
        """check if the goal of the environment is achieved"""
        # TODO make more general. rn for agent1 'TE' agent2

        if self.steps == self.max_episode_length:
            return True

        # agent circle and item circle are close
        if self.no_done:
            return self.steps == self.max_episode_length
        # no done cond or human description
        # if self.no_done or type(self.predicates) is str:
        #     return self.steps == self.max_episode_length

        is_done = True
        for pred in self.predicates:
            agent_id1, agent_id2 = pred[1], pred[2]
            ag1_pos = self.agent_states[agent_id1]["pos"]
            ag2_pos = self.agent_states[agent_id2]["pos"]
            dist = _get_dist(ag1_pos, ag2_pos)
            base_dist = SIZE[self.sizes[agent_id1]] + SIZE[self.sizes[agent_id2]] + 0.5
            if pred[0] == "close":
                pred_done = dist < base_dist
            elif pred[0] == "right":
                pred_done = (ag1_pos[0] > ag2_pos[0]) and (
                    np.abs(ag1_pos[1] - ag2_pos[1]) < base_dist
                )
            elif pred[0] == "above":
                pred_done = (ag1_pos[1] > ag2_pos[1]) and (
                    np.abs(ag1_pos[0] - ag2_pos[0]) < base_dist
                )
            elif pred[0] == "dist":
                target_dist = pred[3]
                pred_done = np.abs(dist - target_dist) < base_dist
            elif pred[0] == "dist_gt":
                target_dist = pred[3]
                pred_done = dist > target_dist
            elif pred[0] == "diag":
                pred_done = (ag1_pos[0] > ag2_pos[0]) and (ag1_pos[1] > ag2_pos[1])
            elif pred[0] == "orien":
                # pdb.set_trace()
                ag1, target_deg1 = pred[1], pred[3]
                ag2, target_deg2 = pred[2], pred[4]
                # in deg (convert from rad) and relative to the line between centers of objects
                agent_deg1 = math.degrees(self.agents[ag1].angle) % 360  # rad to deg
                agent_deg2 = math.degrees(self.agents[ag2].angle) % 360  # rad to deg
                slope = np.sign((ag2_pos[1] - ag1_pos[1]) / (ag2_pos[0] - ag1_pos[0]))
                Y1 = max(ag2_pos[1], ag1_pos[1])
                Y2 = min(ag2_pos[1], ag1_pos[1])
                acute_vec_yaxis = (
                    math.degrees(
                        np.arccos(
                            (Y1 - Y2)
                            / np.sqrt(
                                ((ag2_pos[0] - ag1_pos[0]) ** 2) + ((Y1 - Y2) ** 2)
                            )
                        )
                    )
                    % 360
                )
                if acute_vec_yaxis > 90:
                    acute_vec_yaxis = 180 - acute_vec_yaxis
                axis_trans_deg = -slope * (90 - acute_vec_yaxis)
                rel_deg1 = agent_deg1 + axis_trans_deg
                rel_deg2 = agent_deg2 + axis_trans_deg
                pred_done = (
                    np.abs(rel_deg1 - target_deg2) < 10
                    and np.abs(rel_deg2 - target_deg1) < 10
                )
        ...
        # return self.get_reward_state(0, self.get_state(), 'stop', None, None, self.goals[0]) > -0.02

    def get_done_reward(self, curr_state, agent_id1=0, agent_id2=1):
        """return _get_done as a score, higher-->better"""
        # return -_get_dist(curr_state[agent_id1]['pos'], curr_state[agent_id2]['pos'])
        """sum subgoal rewards"""
        global_rew = 0
        for goal in self.expert_subgoals:
            global_rew += self.get_reward_state(goal[1], curr_state, "stop", goal=goal)
        return global_rew
        # return None
    ...
    def _setup_tmp(self, env_id, curr_state, curr_certainty=None):
        ...
        if self.enable_renderer:
            ...
            pygame.display.set_caption("Maze_v2 belief")
        ...
        for agent_id in range(self.num_agents):
            ...
            self.tmp_repeat_actions = [None] * self.num_agents
    ...
    def _send_action_tmp(self, agent_id, action):
        ...
        self.tmp_agents[agent_id].ApplyLinearImpulse(f * 0.05, p, True)
    ...
    def _step_tmp(self, update_certainty=True):
        ...
        if update_certainty:
            self._update_field_of_view_tmp()
        self.tmp_running = not self._terminal_tmp()
    ...
    def set_state(
        self, agent_states, item_states=[], certainty=None, FOVs=None, steps=0
    ):
        ...
        for agent_id, agent_state in enumerate(agent_states):
            ...
            if ag_shape == "circle":
                ...
                self.agents[agent_id].CreateCircleFixture(
                    radius=R,
                    density=DENSITY[self.densities[agent_id]],
                    friction=FRICTION,
                    restitution=0,
    ...
    #     )
    #     self.agents, self.items = [], []
    #     self.densities = [1, 1] #TODO is consistent with planner density?
    ...
    def get_boundaries(self):
        ...
        return min_x + buffer_x, min_y + buffer_y, max_x - buffer_x, max_y - buffer_y
    ...
    def update_observations_human_simulation(self):
        ...
        for agent_id, agent in enumerate(self.agents):
            ...
            # self.observation_space[agent_id] = self._get_obs(agent)
    ...
    def get_subgoals_avoid(self, agent_id):
        ...
        # print('avoidance subgoal:', dist, 1.5 * SIZE[self.sizes[1 - agent_id]] + 1.5 * SIZE[self.sizes[agent_id]])
        if (
            dist
...
class WM_test(WM):
    def __init__(
        ...
        action_type="force",
        maze_sampler=None,
        goals=[["TE", 0, 1, 1], [None], [None]],
        strengths=[0, 0, 0],
        sizes=[0, 0, 0],
        densities=[0, 0, 0],
        init_positions=[18, 11, 12],  # [3, 6, 14], #TODO randomly sample from POS?
        action_space_types=[0, 0, 0],
        costs=[0, 0, 0],
        temporal_decay=[0, 0, 0],
        visibility=[1, 1, 1],
        ...
        super().__init__(
            ...
            TIME_STEP,
            enable_renderer,
            render_separate_particles,
            random_colors,
            random_colors_agents,
            full_obs,
            n_particles,
            belief_prior,
            init_agent_angles,
            use_all_init_grid_pos,
            human_simulation,
        )
        self.no_done = no_done
    ...
    def _get_done(self, agent_id1, agent_id2):
        ...
        if (
            _get_dist(
                self.agent_states[agent_id1]["pos"], self.agent_states[agent_id2]["pos"]
            )
            < SIZE[self.sizes[agent_id1]] + SIZE[self.sizes[agent_id2]] + 0.5
        ):
            return True
