...
import collections
from typing import Deque, Iterable, Mapping, Union
...
class RewardVecEnvWrapper(vec_env.VecEnvWrapper):
    def __init__(
        ...
        ep_history: int = 100,
        G: np.ndarray = None,
        expert_data: Union[Iterable[Mapping], types.Transitions] = None,
    ):
    ...
    def step_wait(self):
        ...
        # import inspect
        # print('self.reward_fn',inspect.getfullargspec(self.reward_fn))
        # print(type(self.reward_fn))
        # pdb.set_trace()
        rews, graphs = self.reward_fn(
            self._old_obs,
            self._actions,
            obs_fixed,
            np.array(dones),
            G=self.G,
            all_demos=self.expert_data,
        )
        # print('step_wait rews.requires_grad', rews.requires_grad) #rews is a numpy
...
class RewardGraphVecEnvWrapper(vec_env.VecEnvWrapper):
    def __init__(
        self,
        venv: vec_env.VecEnv,
        reward_fn: common.RewardFn,
    ...
    def step_wait(self):
        ...
        for info_dict, old_rew in zip(infos, old_rews):
            info_dict["wrapped_env_rew"] = old_rew
        return obs, rews, graphs, dones, infos
