...
class DiscrimNet(nn.Module, abc.ABC):
    ...
    def _eval_reward(
        ...
        if all_demos is not None:
            all_demos = th.as_tensor(
                all_demos, device=self.device()
            ).float()  # TODO add to disc_rew_preprocess_inputs
        if not with_grad:
            with th.no_grad():
                if is_train:
                    rew_th = self.reward_train(
                        state_th, action_th, next_state_th, done_th, G_th, all_demos
                    )
                else:
                    rew_th = self.reward_test(
                        state_th, action_th, next_state_th, done_th, G_th, all_demos
                    )
        else:
            if is_train:
...
class DiscrimNetAIRL(DiscrimNet):
    ...
    def logits_gen_is_high(
        ...
        predicting expert.
        """
        output = self.reward_net.reward_train(
            state, action, next_state, done, G, all_demos
        )
        if self.reward_net.use_graph:
            reward_output_train, graph_output_train = output
        else:
            reward_output_train = output
            graph_output_train = None        
    ...
    def reward_test(
        ...
        rew, graph = self.reward_net.reward_test(
            state, action, next_state, done, G, all_demos, return_value, n_entities, state_dim
        )
    ...
    def reward_train(
        ...
        # print('discrim nets reward train', all_demos)
        output = self.reward_net.reward_train(
            state, action, next_state, done, G, all_demos
        )
        if self.reward_net.use_graph:
    ...
    def _eval_reward(
        ...
        if not with_grad:
            with th.no_grad():
                if is_train:
                    output_th = self.reward_train(
                        state_th, action_th, next_state_th, done_th, G_th, all_demos
                    )
                else:
                    output_th = self.reward_test(
                        state_th, action_th, next_state_th, done_th, G_th, all_demos, return_value, n_entities, state_dim
                    )
        else:
            if is_train:
                output_th = self.reward_train(
        ...
        rew_th, graph = output_th
        if self.reward_net.use_graph:            
            graph = graph.detach().cpu().numpy() #otherwise None
        rew = rew_th.detach().cpu().numpy().flatten()
        assert rew.shape == (len(state),)
