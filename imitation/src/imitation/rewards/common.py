...
def _reward_fn_normalize_inputs(
    ...
    *,
    ...
    norm_next_obs = (
        vec_normalize.normalize_obs(next_obs) if next_obs is not None else None
    )
    rew = reward_fn(
        norm_obs,
        acts,
...
def _reward_fn_normalize_inputs_no_demos(
    obs: np.ndarray,
    # acts: np.ndarray = (None,)
    # next_obs: np.ndarray = None,
    # dones: np.ndarray = None,
    ...
    *,
    ...
    # )
    state_th = (
        th.tensor(norm_obs.reshape((norm_obs.shape[0], num_nodes, -1)))
        .float()
        .to("cuda")
    )
    G_th = th.tensor(G).float().to("cuda")
    rew = reward_fn(state_th, G=G_th)
    if norm_reward:
        rew = vec_normalize.normalize_reward(rew)
    return rew


def _reward_fn_no_demos(
    obs: np.ndarray = None,
    ...
    *,
    ...
    """Combine with `functools.partial` to create a RewardFn.
    ...
    if obs is not None:
        state_th = (
            th.tensor(obs.reshape((obs.shape[0], num_nodes, -1))).float().to("cuda")
        )
        rew = reward_fn(state_th, G=G_th)
    else:
        edge_th = th.tensor(edges).float().to("cuda")
        rew = reward_fn(edge_rep=state_th, G=G_th)
    return rew
...
def build_reward_fn(*, reward_fn, **kwargs) -> RewardFn:
    """Reward function that automatically normalizes inputs.
...
def build_norm_reward_fn_no_demos(*, reward_fn, vec_normalize, **kwargs) -> RewardFn:
    """Reward function that automatically normalizes inputs.

    See _reward_fn_normalize_inputs for argument documentation.
    """
    return functools.partial(
        _reward_fn_normalize_inputs_no_demos,
        reward_fn=reward_fn,
        vec_normalize=vec_normalize,
        **kwargs,
    )
...
def build_reward_fn_no_demos(*, reward_fn, **kwargs) -> RewardFn:
    """Reward function.

    See _reward_fn_no_demos for argument documentation.
    """
    return functools.partial(
        _reward_fn_no_demos,
        reward_fn=reward_fn,
...
def disc_rew_preprocess_inputs(
    ...
    if G is not None:
        # print('disc_rew_preprocess_inputs',type(G))
        G_th = th.as_tensor(G, device=device)
    else:
        G_th = None
    ...
    return state_th, action_th, next_state_th, done_th, G_th
...
def compute_train_stats(
    ...
    with th.no_grad():
        ...
        # print('disc_logits_gen_is_high',disc_logits_gen_is_high)
