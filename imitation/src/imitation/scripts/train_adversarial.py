...
from imitation.data.IRL_types import Trajectory
from imitation.planner.planner_v1.planner import Planner

import stable_baselines3


def save(trainer, save_path, policy_type=None):
...
def train(
    ...
    policy_type: str,
    # G_path: str,
    # reward_net_kwargs: Mapping[str, Mapping],
) -> dict:
    ...
    print(init_rl_kwargs)
    n_steps = init_rl_kwargs["n_steps"]
    if init_rl_kwargs["model_class"] != stable_baselines3.ppo.ppo.PPO:
        init_rl_kwargs = init_rl_kwargs = dict(
            policy_class=init_rl_kwargs["policy_class"],
            model_class=init_rl_kwargs["model_class"],
        )
    if policy_type == "planner":
        random.seed(_seed)
        np.random.seed(_seed)
        gen_algo = Planner(
            env_name, log_dir, beta=algorithm_kwargs.get(algorithm, {}).get("beta")
        )
    else:
    ...
    if init_rl_kwargs["policy_class"] != stable_baselines3.PPO:
        gen_algo.n_steps = n_steps
    ...
    def callback(round_num):
        if checkpoint_interval > 0 and round_num % checkpoint_interval == 0:
            save(
                trainer,
                os.path.join(log_dir, "checkpoints", f"{round_num:05d}"),
                policy_type,
            )

    # with open(G_path, "rb") as input_file:
    #     G = pickle.load(input_file)
    # trainer.train(total_timesteps, callback, G)
    trainer.train(total_timesteps, callback)

    # Save final artifacts.
    if checkpoint_interval >= 0:
        save(trainer, os.path.join(log_dir, "checkpoints", "final"), policy_type)

    # Final evaluation of imitation policy.
    if policy_type != "planner":  # no eval for planner
        results = {}
