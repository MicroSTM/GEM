...
@train_ex.config
def train_defaults():
    env_name = "PHASE-v0"  # environment to train on
    total_timesteps = 5e5 #4e6  # Num of environment transitions to sample
    ...
    avg_edges = False
    fc_G = False
    reward_type = 'basic' #'GNN'
    if reward_type == 'basic':
        reward_net_cls = reward_nets.BasicShapedRewardNet
    elif reward_type == 'GNN':
        reward_net_cls = reward_nets.GNNShapedRewardNet
    ...
    algorithm_kwargs = dict(
        shared=dict(
            expert_batch_size=expert_batch_size,  # Number of expert samples per discriminator update
            # Number of discriminator updates after each round of generator updates
            n_disc_updates_per_round=4,
            G_final_state=G_final_state
        ),
        airl=dict(reward_net_cls=reward_net_cls, 
                  reward_net_kwargs=dict(use_action=use_action, use_graph=use_graph, 
                                         use_attention=use_attention, avg_edges=avg_edges,
                                         fc_G=fc_G), #, n_entities=n_entities, state_dim=state_dim
                  beta=beta,
                  debug_use_ground_truth=debug_use_ground_truth,
    ...
    # use_action = True
    # reward_net_kwargs = dict(use_action=use_action)

    policy_type = 'ppo'
    model_class, policy_class = None, None
    if policy_type == 'ppo':
        model_class = stable_baselines3.PPO
        policy_class = base.FeedForward32Policy #original default. in expert demos it's ActorCriticPolicy
    elif policy_type == 'sac':
        model_class = stable_baselines3.SAC
        policy_class = SACPolicy
        DEFAULT_INIT_RL_KWARGS = dict()
    elif policy_type == 'td3':
        model_class = stable_baselines3.TD3 
...
@train_ex.config
def calc_n_steps(num_vec, gen_batch_size, policy_type):
    # if policy_type == 'ppo':
