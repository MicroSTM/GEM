...
def rollouts_and_policy(
    ...
    *,
    ...
    if init_tensorboard:
        ...
        if isinstance(init_rl_kwargs, str):
            init_rl_kwargs = json.loads(init_rl_kwargs)
        else:
    ...
    if reward_type is not None:
        ...
        import inspect
        # print('reward_fn',inspect.getfullargspec(reward_fn))    
        # print(reward_fn)
        # print(type(reward_fn))    
        #TODO see adversarial.py on preparing expert demos for train_adversarial.
        expert_trajs = types.load(rollout_path) #list of TrajectoryWithRew?
        expert_trajs = expert_trajs[0].obs[None,:,:] #TODO multiple demos
        venv = RewardVecEnvWrapper(venv, reward_fn, expert_data=expert_trajs)
    ...
    # print('init_rl_kwargs', init_rl_kwargs)
